	...
	$.jstree.plugin("dnd", {
		...
		_fn : {
			...
			dnd_show : function () {
				...
				switch(r) {
					case "before":
						m.addClass('before').removeClass('after').removeClass('inside').css({ "cursor": "cancel", "left" : pos - 20 + "px", "top" : (this.data.dnd.off.top - 6) + "px" }).show();
						break;
					case "after":
						m.addClass('after').removeClass('before').removeClass('inside').css({ "left" : pos - 20 + "px", "top" : (this.data.dnd.off.top + this.data.core.li_height - 7) + "px" }).show();
						break;
					case "inside":
						m.addClass('inside').removeClass('after').removeClass('before').css({ "left" : pos - 8 + "px", "top" : (this.data.dnd.off.top + this.data.core.li_height/2 - 6) + "px" }).show();
						break;
					default:
						m.hide();
						break;
